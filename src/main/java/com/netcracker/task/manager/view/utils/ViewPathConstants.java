package com.netcracker.task.manager.view.utils;


public class ViewPathConstants {
    public static final String PATH_TO_MAIN_FORM_VIEW         = "/mainForm/mainFormView.fxml";
    public static final String PATH_TO_ADD_FORM_VIEW          = "/addForm/addTaskView.fxml";
    public static final String PATH_TO_EDIT_FORM_VIEW         = "/editForm/editTaskView.fxml";
    public static final String PATH_TO_NOTIFICATION_FORM_VIEW = "/notificationForm/notificationTaskView.fxml";
    public static final String PATH_TO_POSTPONE_FORM_VIEW     = "/notificationForm/postponeForm/postponeTaskView.fxml";
    
    public static final String PATH_TO_EDIT_BUTTON_IMAGE = "/images/editButton.png";
}
